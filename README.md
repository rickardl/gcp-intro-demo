...
I have added a unique identifier to be added to the project ids (see [gcp_env_template.sh](gcp_env_template.sh) - ```GCP_ADMIN_VERSION``` and ```GCP_INFRA_VERSION```). The reason for this is that GCP does not allow you to use the same project id even if you deleted the project with that project id earlier - so, if you delete the project and create the project again - you need to give a new unique project id. Therefore increment the project id numbers in your environment file, and source the file before running the [create-admin-proj.sh](create-admin-proj.sh) script for creating the admin project or running terraform init/get/plan/apply for the infra project. For the infra project I could have let Terraform to generate a unique identifier and add it to the project id. But using an external environment variable I could automate also how to create the gcloud configuration (see: [create-infra-configuration.sh](create-infra-configuration.sh)).
